(function () {
  "use strict";
  var JADWAL_OTOMATIS_URL ="https://a3d4pt0.vendorscore.live/api/pialadunia/matches";
  ...
  var HANYA_MOBILE =typeof window !== "undefined" &&
...
border: 2px solid #f5c542 !important;
  ...
  function normalisasiMatch(raw) {
    ...
    var kickoff = teksBersih(
      nilaiPertamaTerisi(raw.kickoff, raw.utcDate, raw.utc_date, raw.matchDate)
    );

    var waktuWIB = formatTanggalJamWIB(kickoff);

    var tanggal = teksBersih(
      nilaiPertamaTerisi(
        raw.tanggal,
        raw.date,
        raw.match_date,
  ...
  function updateIsiPopup(popup, match, info) {
    ...
    var scoreText = angkaSkor(match.homeScore) + " : " + angkaSkor(match.awayScore);
